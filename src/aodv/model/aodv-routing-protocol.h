...
#include "ns3/double-probe.h" // added for double tracing
...
/****************************************Ipv6 AodvRoutingProtocol****************************************/



class RoutingProtocol6 : public Ipv6RoutingProtocol
{
public:
  static TypeId GetTypeId (void);
  static const uint32_t AODV_PORT;
  ...
  /// c-tor
  RoutingProtocol6 ();
  virtual ~RoutingProtocol6();
  virtual void DoDispose ();

  // Inherited from Ipv6RoutingProtocol
  Ptr<Ipv6Route> RouteOutput (Ptr<Packet> p, const Ipv6Header &header, Ptr<NetDevice> oif, Socket::SocketErrno &sockerr);	//the 'packet' calls GetDeserializedSize function, which needs to be taken care of
...
private:
  /**
    * The nHops value will be traced during the simulation
    */

//   TracedValue <uint32_t> m_requestId;
  ...
  /// Routing table
  RoutingTable6 m_routingTable;
  /// A "drop-front" queue used by the routing layer to buffer packets to which it does not have a route.
  RequestQueue6 m_queue;
  /// Broadcast ID
  // uint32_t m_requestId;
  TracedValue <uint32_t> m_requestId;
  /// Request sequence number
  uint32_t m_seqNo;
  /// Handle duplicated RREQ
  IdCache6 m_rreqIdCache;
  /// Handle duplicated broadcast/multicast packets
  DuplicatePacketDetection6 m_dpd;
  /// Handle neighbors
  Neighbors6 m_nb;
  /// Number of RREQs used for RREQ rate control
  uint16_t m_rreqCount;
  /// Number of RERRs used for RERR rate control
  uint16_t m_rerrCount;
  TracedValue <double> m_nhops;
  TracedValue <double> m_RequestsReceived;
...
private:
  ...
  */
  ...
  /// Check that packet is send from own interface
  bool IsMyOwnAddress (Ipv6Address src,uint32_t index);

  bool IsDestinationAddress (Ipv6Address address, uint32_t iif);
  /// Find unicast socket with local interface address iface
  Ptr<Socket> FindSocketWithInterfaceAddress (Ipv6InterfaceAddress iface) const;
  Ptr<Socket> FindSocketWithInterfaceAddress (Ipv6InterfaceAddress iface, uint32_t iif) const;
  /// Find subnet directed broadcast socket with local interface address iface
  Ptr<Socket> FindSubnetBroadcastSocketWithInterfaceAddress (Ipv6InterfaceAddress iface) const;
  /// Process hello message
  void ProcessHello (RrepHeader6 const & rrepHeader, Ipv6Address receiverIfaceAddr);
  /// Create loopback route for given header
  ...
  void RecvAodv (Ptr<Socket> socket);
  /// Receive RREQ
  void RecvRequest (Ptr<Packet> p, Ipv6Address receiver, Ipv6Address src);	//the 'packet' calls GetDeserializedSize function, which needs to be taken care of
  /// Receive RREP
  ...
  /// Send RREP
  void SendReply (RreqHeader6 const & rreqHeader, RoutingTableEntry6 const & toOrigin);
  /** Send RREP by intermediate node
   * \param toDst routing table entry to destination
   * \param toOrigin routing table entry to originator
   * \param gratRep indicates whether a gratuitous RREP should be unicast to destination
   */
  void SendReplyByIntermediateNode (RoutingTableEntry6 & toDst, RoutingTableEntry6 & toOrigin, bool gratRep);
  /// Send RREP_ACK
  void SendReplyAck (Ipv6Address neighbor);
  /// Initiate RERR
  void SendRerrWhenBreaksLinkToNextHop (Ipv6Address nextHop);
  /// Forward RERR
  void SendRerrMessage (Ptr<Packet> packet,  std::vector<Ipv6Address> precursors); //the 'packet' calls GetDeserializedSize function, which needs to be taken care of
  /**
   * Send RERR message when no route to forward input packet. Unicast if there is reverse route to originating node, broadcast otherwise.
   * \param dst - destination node IP address
   * \param dstSeqNo - destination node sequence number
