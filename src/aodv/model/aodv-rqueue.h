...
public:
  ...
  /// c-tor
  QueueEntry6 (Ptr<const Packet> pa = 0, Ipv6Header const & h = Ipv6Header (),
              UnicastForwardCallback ucb = UnicastForwardCallback (),
              ErrorCallback ecb = ErrorCallback (), Time exp = Simulator::Now ()) :
    m_packet (pa), m_header (h), m_ucb (ucb), m_ecb (ecb),
    m_expire (exp + Simulator::Now ())
   ...
   * \return true if equal
   */
  bool operator== (QueueEntry6 const & o) const
  {
...
public:
  /// Default c-tor
  RequestQueue6 (uint32_t maxLen, Time routeToQueueTimeout) :
    m_maxLen (maxLen), m_queueTimeout (routeToQueueTimeout)
  {
  }
  /// Push entry in queue, if there is no entry with the same packet and destination address in queue.
  bool Enqueue (QueueEntry6 & entry);
  /// Return first found (the earliest) entry for given destination
  bool Dequeue (Ipv6Address dst, QueueEntry6 & entry);
  /// Remove all packets with destination IP address dst
...
private:
  ...
  std::vector<QueueEntry6> m_queue;
  /// Remove all expired entries
  void Purge ();
  /// Notify that packet is dropped from queue by timeout
  void Drop (QueueEntry6 en, std::string reason);
  /// The maximum number of packets that we allow a routing protocol to buffer.
  uint32_t m_maxLen;
  /// The maximum period of time that a routing protocol is allowed to buffer a packet for, seconds.
  Time m_queueTimeout;
  static bool IsEqual (QueueEntry6 en, const Ipv6Address dst) { return (en.GetIpv6Header ().GetDestinationAddress () == dst); }
};
