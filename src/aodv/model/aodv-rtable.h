...
class RoutingTableEntry6
{
public:
  /// c-to
  RoutingTableEntry6 (Ptr<NetDevice> dev = 0,Ipv6Address dst = Ipv6Address (), bool vSeqNo = false, uint32_t m_seqNo = 0,
  ...
  // Fields
  Ipv6Address GetDestination () const { return m_ipv6Route->GetDestination (); }
  Ptr<Ipv6Route> GetRoute () const { return m_ipv6Route; }
  void SetRoute (Ptr<Ipv6Route> r) { m_ipv6Route = r; }
  void SetNextHop (Ipv6Address nextHop) { m_ipv6Route->SetGateway (nextHop); }
  Ipv6Address GetNextHop () const { return m_ipv6Route->GetGateway (); }
  void SetOutputDevice (Ptr<NetDevice> dev) { m_ipv6Route->SetOutputDevice (dev); }
  Ptr<NetDevice> GetOutputDevice () const { return m_ipv6Route->GetOutputDevice (); }
  Ipv6InterfaceAddress GetInterface () const { return m_iface; }
  void SetInterface (Ipv6InterfaceAddress iface) { m_iface = iface; }
  void SetValidSeqNo (bool s) { m_validSeqNo = s; }
  bool GetValidSeqNo () const { return m_validSeqNo; }
  void SetSeqNo (uint32_t sn) { m_seqNo = sn; }
  uint32_t GetSeqNo () const { return m_seqNo; }
...
public:
   ...
   * \return true in success
   */
  bool AddRoute (RoutingTableEntry6 & r);
  /**
   * Delete routing table entry with destination address dst, if it exists.
   * \param dst destination address
   * \return true on success
   ...
   * \return true on success
   */
  bool LookupRoute (Ipv6Address dst, RoutingTableEntry6 & rt);
  /// Lookup route in VALID state
  bool LookupValidRoute (Ipv6Address dst, RoutingTableEntry6 & rt);
  /// Update routing table
  bool Update (RoutingTableEntry6 & rt);
  /// Set routing table entry flags
...
private:
  std::map<Ipv6Address, RoutingTableEntry6> m_ipv6AddressEntry;
  /// Deletion time for invalid routes
  Time m_badLinkLifetime;
  /// const version of Purge, for use by Print() method
  void Purge (std::map<Ipv6Address, RoutingTableEntry6> &table) const;
};
