...
bool
RequestQueue6::Enqueue (QueueEntry6 & entry)
{
  Purge ();
  for (std::vector<QueueEntry6>::const_iterator i = m_queue.begin (); i
       != m_queue.end (); ++i)
    {
      if ((i->GetPacket ()->GetUid () == entry.GetPacket ()->GetUid ())
...
void
RequestQueue6::DropPacketWithDst (Ipv6Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  Purge ();
  for (std::vector<QueueEntry6>::iterator i = m_queue.begin (); i
       != m_queue.end (); ++i)
    {
      if (IsEqual (*i, dst))
        {
  ...
  m_queue.erase (std::remove_if (m_queue.begin (), m_queue.end (),
                                 std::bind2nd (std::ptr_fun (RequestQueue6::IsEqual), dst)), m_queue.end ());
}
...
bool
RequestQueue6::Dequeue (Ipv6Address dst, QueueEntry6 & entry)
{
  Purge ();
  for (std::vector<QueueEntry6>::iterator i = m_queue.begin (); i != m_queue.end (); ++i)
    {
...
bool
RequestQueue6::Find (Ipv6Address dst)
{
  for (std::vector<QueueEntry6>::const_iterator i = m_queue.begin (); i
       != m_queue.end (); ++i)
    {
...
{
  bool
  operator() (QueueEntry6 const & e) const
  {
    return (e.GetExpireTime () < Seconds (0));
  }
};
...
void
RequestQueue6::Drop (QueueEntry6 en, std::string reason)
{
