...
RoutingTableEntry6::RoutingTableEntry6 (Ptr<NetDevice> dev, Ipv6Address dst, bool vSeqNo, uint32_t seqNo,
...
RoutingTableEntry6::~RoutingTableEntry6 ()
{
}
...
bool
RoutingTableEntry6::InsertPrecursor (Ipv6Address id)
{
  NS_LOG_FUNCTION (this << id);
  if (!LookupPrecursor (id))
    {
...
bool
RoutingTableEntry6::LookupPrecursor (Ipv6Address id)
{
  NS_LOG_FUNCTION (this << id);
...
bool
RoutingTableEntry6::DeletePrecursor (Ipv6Address id)
{
  NS_LOG_FUNCTION (this << id);
...
void
RoutingTableEntry6::DeleteAllPrecursors ()
{
  NS_LOG_FUNCTION (this);
  m_precursorList.clear ();
}
...
bool
RoutingTableEntry6::IsPrecursorListEmpty () const
{
  return m_precursorList.empty ();
}
...
void
RoutingTableEntry6::GetPrecursors (std::vector<Ipv6Address> & prec) const
{
  NS_LOG_FUNCTION (this);
  if (IsPrecursorListEmpty ())
    return;
...
void
RoutingTableEntry6::Invalidate (Time badLinkLifetime)
{
  NS_LOG_FUNCTION (this << badLinkLifetime.GetSeconds ());
  if (m_flag == INVALID)
    return;
...
void
RoutingTableEntry6::Print (Ptr<OutputStreamWrapper> stream) const
{
  std::ostream* os = stream->GetStream ();
  *os << m_ipv6Route->GetDestination () << "\t" << m_ipv6Route->GetGateway ()
      << "\t" << m_iface.GetAddress() << "\t";
  switch (m_flag)
    {
    case VALID:
...
RoutingTable6::RoutingTable6 (Time t) :
  m_badLinkLifetime (t)
{
}
...
bool
RoutingTable6::LookupRoute (Ipv6Address id, RoutingTableEntry6 & rt)
{
  NS_LOG_FUNCTION (this << id);
  Purge ();
  ...
  std::map<Ipv6Address, RoutingTableEntry6>::const_iterator i =
...
bool
RoutingTable6::LookupValidRoute (Ipv6Address id, RoutingTableEntry6 & rt)
{
  NS_LOG_INFO("RoutingTable6 LookupValidRoute is called for id="<<id);
  NS_LOG_FUNCTION (this << id);
  if (!LookupRoute (id, rt))
    {
      NS_LOG_LOGIC ("Route to " << id << " not found");
  ...
  NS_LOG_LOGIC ("Route to " << id << " flag is " << ((rt.GetFlag () == VALID) ? "valid" : "not valid"));
  NS_LOG_INFO ("Route to " << id << " flag is " << ((rt.GetFlag () == VALID) ? "valid" : "not valid"));
  return (rt.GetFlag () == VALID);
}
...
bool
RoutingTable6::DeleteRoute (Ipv6Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  Purge ();
...
bool
RoutingTable6::AddRoute (RoutingTableEntry6 & rt)
{
  NS_LOG_FUNCTION (this);
  Purge ();
  if (rt.GetFlag () != IN_SEARCH)
    rt.SetRreqCnt (0);
  std::pair<std::map<Ipv6Address, RoutingTableEntry6>::iterator, bool> result =
    m_ipv6AddressEntry.insert (std::make_pair (rt.GetDestination (), rt));
  NS_ASSERT_MSG(result.second, "Routing entry not inserted");
  NS_LOG_INFO("Add Route: dst="<<rt.GetDestination()<<" interface address="<<rt.GetInterface()<<" Nxt hop="<<rt.GetNextHop()<<" seq no="<<rt.GetSeqNo());
  NS_LOG_INFO("RoutingTable6 AddRoute returns="<<result.second<<" result.first="<<result.first->second.GetInterface());
...
bool
RoutingTable6::Update (RoutingTableEntry6 & rt)
{
  NS_LOG_FUNCTION (this);
  std::map<Ipv6Address, RoutingTableEntry6>::iterator i =
...
bool
RoutingTable6::SetEntryState (Ipv6Address id, RouteFlags state)
{
  NS_LOG_FUNCTION (this);
  std::map<Ipv6Address, RoutingTableEntry6>::iterator i =
    m_ipv6AddressEntry.find (id);
...
void
RoutingTable6::GetListOfDestinationWithNextHop (Ipv6Address nextHop, std::map<Ipv6Address, uint32_t> & unreachable )
{
  NS_LOG_FUNCTION (this);
  Purge ();
  unreachable.clear ();
  for (std::map<Ipv6Address, RoutingTableEntry6>::const_iterator i =
...
void
RoutingTable6::InvalidateRoutesWithDst (const std::map<Ipv6Address, uint32_t> & unreachable)
{
  NS_LOG_FUNCTION (this);
  Purge ();
  for (std::map<Ipv6Address, RoutingTableEntry6>::iterator i =
         m_ipv6AddressEntry.begin (); i != m_ipv6AddressEntry.end (); ++i)
    {
...
void
RoutingTable6::DeleteAllRoutesFromInterface (Ipv6InterfaceAddress iface)
{
  NS_LOG_FUNCTION (this);
  if (m_ipv6AddressEntry.empty ())
    return;
  for (std::map<Ipv6Address, RoutingTableEntry6>::iterator i =
         m_ipv6AddressEntry.begin (); i != m_ipv6AddressEntry.end ();)
    {
      if (i->second.GetInterface () == iface)
        {
          std::map<Ipv6Address, RoutingTableEntry6>::iterator tmp = i;
          ++i;
...
void
RoutingTable6::Purge (std::map<Ipv6Address, RoutingTableEntry6> &table) const
{
  NS_LOG_FUNCTION (this);
  if (table.empty ())
    return;
...
bool
RoutingTable6::MarkLinkAsUnidirectional (Ipv6Address neighbor, Time blacklistTimeout)
{
  NS_LOG_FUNCTION (this << neighbor << blacklistTimeout.GetSeconds ());
  std::map<Ipv6Address, RoutingTableEntry6>::iterator i =
...
void
RoutingTable6::Print (Ptr<OutputStreamWrapper> stream) const
{
  std::map<Ipv6Address, RoutingTableEntry6> table = m_ipv6AddressEntry;
  Purge (table);
  *stream->GetStream () << "\nAODV Routing table\n"
                        << "Destination\tGateway\t\tInterface\tFlag\tExpire\t\tHops\n";
  for (std::map<Ipv6Address, RoutingTableEntry6>::const_iterator i =
