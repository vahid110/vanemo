  ...
  if (senderAddress == "fe80::200:ff:fe00:4")
    {
      ...
      std::list<RipNgRte>::iterator it(rtes.begin());
      while(it != rtes.end())
    	  NS_LOG_UNCOND(*it++ << "\n\n");
...
Ipv6RipngSplitHorizonStrategyTest::DoRun (void)
{
  ...
  RipNgHelper ripNgRouting;
  ripNgRouting.Set ("SplitHorizon", EnumValue (RipNg::NO_SPLIT_HORIZON));
  ...
  Ptr<Socket> rxSocket = rxSocketFactory->CreateSocket ();
  rxSocket->Bind (Inet6SocketAddress (Ipv6Address ("ff02::9"), 521));
...
int main()
{
	Ipv6RipngSplitHorizonStrategyTest r(RipNg::NO_SPLIT_HORIZON);
	r.DoRun();
}
