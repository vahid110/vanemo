...
main (int argc, char *argv[])
{
  ...
  ueIpIfaces = epcHelper->AssignUeIpv6Address (NetDeviceContainer (ueLteDevs));
  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      ueStaticRouting->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress6 (), (uint32_t)1);
    }
      ...
      for (uint32_t b = 0; b < numBearersPerUe; ++b)
        {
          ...
          UdpClientHelper dlClientHelper (ueIpIfaces.GetAddress (u, 1), dlPort);
