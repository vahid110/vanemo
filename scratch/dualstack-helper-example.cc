...
int main(int argc, char** argv) {
	...
	NetDeviceContainer c4 = csma.Install(ipv4Only);
	...
	addrhelp.SetIpv4Base("192.88.99.0", "255.255.255.0", "0.0.0.1");
	DualStackContainer i4 = addrhelp.AssignIpv4Only(c4);
	i46.SetDefaultRouteInAllNodes(0);
	i4.SetForwarding(0, true);
	i6.SetForwarding(1, true);
	i46.SetForwarding(0, true);
	i46.SetForwarding(1, true);

	Ipv4GlobalRoutingHelper::PopulateRoutingTables ();

	Ipv6StaticRoutingHelper routingHelper6;
	  Ptr<OutputStreamWrapper> routingStream6 = Create<OutputStreamWrapper> (&std::cout);
	...
	UdpEchoServerHelper echoServer(10);
	 ...
	 UdpEchoClientHelper echoClient (i4.GetIpv4Address(1), 10);
	 echoClient.SetAttribute ("MaxPackets", UintegerValue (1));
	 echoClient.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
	 echoClient.SetAttribute ("PacketSize", UintegerValue (1024));

	 ApplicationContainer clientApps = echoClient.Install (r);
